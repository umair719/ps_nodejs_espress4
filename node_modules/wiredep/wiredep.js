...
function wiredep(opts) {
    ...
    config.set
    ('on-error', opts.onError || function (err) {
            throw new Error(err);
        })
    ...
    $._.map(config.get('file-types'), 'detect').forEach(function (fileType) {
    ...
    return config.get('stream').src ||
        Object.keys(config.get('global-dependencies-sorted')).reduce(function (acc, depType) {
